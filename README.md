...
node scripts/verify.mjs --static   # 資料、來源 ID、schema、示範引擎驗收、下載檔、機敏字串
node scripts/verify.mjs            # 加上 Playwright 瀏覽器檢查（導覽、搜尋、篩選、複製、下載、案例互動、行動裝置）
python3 skills/claude/preemptive-exposure-analysis/scripts/validate_inputs.py examples/synthetic-org
```

實際檢查結果記錄於 `verify-report.md`。
