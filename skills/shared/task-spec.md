...
- 信心水準：缺 0 類輸入 → 高；缺 1–2 類 → 中；缺 3 類以上或資產清冊完整度 < 80% → 低。
...
1. 讀入 9 個檔案並回報筆數（assets 12、vulnerabilities 14、identities 8、misconfigurations 9、controls 8、actors 3、edges 19、exposures 7）。
...
impact = clamp(business_criticality·1.4 + data_weight(Restricted=3, Confidential=2, Internal=1) + 2·[crown_jewel] + 2·[entry_to_crown_jewel], 0, 10)
score = likelihood × impact  (0–100)
priority: balanced → P1 ≥ 60, P2 ≥ 40, P3 ≥ 20, 其餘 P4
