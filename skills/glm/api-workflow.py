#!/usr/bin/env python3
"""
GLM（Zhipu AI / Z.ai）API 工作流程：先制型曝險分析（Preemptive Exposure Analysis）

用途
  以 skills/shared/core-prompt.md 作為 system prompt，讀入 examples/synthetic-org/ 的 9 個輸入檔，
  透過 GLM 的 OpenAI 相容端點（預設）或 Anthropic 相容端點（--anthropic-compatible，待驗證）
  產出 Markdown 報告與符合 output-schema.json 的 JSON。

兩段式呼叫
  第 1 段：產出人可讀的 Markdown 報告（正體中文）。
  第 2 段：延續同一對話，只輸出符合 output-schema.json 的 JSON 物件。

環境變數（勿寫入程式碼、勿提交到 git）
  GLM_API_KEY        必要（--dry-run 時不需要）
  GLM_BASE_URL       預設 https://api.z.ai/api/paas/v4（國際版 Z.ai API Platform）
                     中國大陸 BigModel 可改為 https://open.bigmodel.cn/api/paas/v4
                     ※ 正確的 base URL 請以 docs.z.ai 或 open.bigmodel.cn 官方文件為準（本檔撰寫時官方站台無法直接查證）。
  GLM_MODEL          預設 glm-5.3（※ 模型 ID 請以官方模型列表確認；zai-org/GLM-5 README 記載 GLM-5.3 / GLM-5.3-Flash）
  GLM_ANTHROPIC_BASE_URL  預設 https://api.z.ai/api/anthropic（僅 --anthropic-compatible 使用；PARTIALLY VERIFIED）

用法
  python3 skills/glm/api-workflow.py --dry-run                 # 只組裝提示詞，不呼叫 API、不需 SDK
  python3 skills/glm/api-workflow.py                           # OpenAI 相容端點（需 pip install openai）
  python3 skills/glm/api-workflow.py --anthropic-compatible    # Anthropic 相容端點（需 pip install anthropic；待驗證）
  python3 skills/glm/api-workflow.py --input-dir /path/to/dir  # 以自己的資料夾取代合成資料

輸出
  output/glm-report.md、output/glm-output.json（JSON 解析失敗時另存 output/glm-output.raw.txt）
...
PLATFORM = "glm"
DEFAULT_BASE_URL = "https://api.z.ai/api/paas/v4"           # 請以 docs.z.ai 官方文件確認
ALT_BASE_URL_CN = "https://open.bigmodel.cn/api/paas/v4"    # BigModel（中國大陸）替代端點，請以官方文件確認
...
def load_core_prompt() -> str:
    ...
    platform_note = (
        "\n\n7. 平台備註（GLM）\n"
        "- 你正透過 Z.ai / BigModel API 被呼叫，沒有檔案系統或工具；所有輸入都已貼在使用者訊息中。\n"
...
def build_json_prompt(schema_text: str) -> str:
    return (
        "請把上一則報告的內容轉為**單一 JSON 物件**，嚴格符合下列 JSON Schema（output-schema.json）。"
        "只輸出 JSON，不要 Markdown 圍欄、不要任何說明文字。"
        "meta.generated_by 請填「GLM via " + PLATFORM + " api-workflow.py」；"
...
def call_openai_compatible(system_prompt: str, user_prompt: str, json_prompt: str,
                           model: str, max_tokens: int) -> tuple[str, str]:
    try:
        from openai import OpenAI  # type: ignore
    except ImportError:
        sys.exit("[錯誤] 未安裝 openai SDK。請執行：pip install -r skills/glm/requirements.txt")

    api_key = os.environ.get("GLM_API_KEY")
    if not api_key:
        sys.exit("[錯誤] 未設定環境變數 GLM_API_KEY（請勿寫入程式碼或提交到 git）。")
    base_url = os.environ.get("GLM_BASE_URL", DEFAULT_BASE_URL)
    client = OpenAI(api_key=api_key, base_url=base_url)
    print(f"[資訊] OpenAI 相容端點：{base_url}，模型：{model}", file=sys.stderr)
    ...
    # 第 2 段：延續對話，只輸出 JSON。
    # 註：GLM 是否支援 response_format={"type":"json_object"} 官方文件未能查證（待驗證），
    #     因此這裡以提示詞要求純 JSON，並在存檔前做寬鬆解析。
    messages += [
...
def call_anthropic_compatible(system_prompt: str, user_prompt: str, json_prompt: str,
                              model: str, max_tokens: int) -> tuple[str, str]:
    """Anthropic 相容端點（GLM Coding Plan 所用）。狀態：PARTIALLY VERIFIED／待驗證。
    注意：第三方引述的 Z.ai 條款指出 Coding Plan 額度僅限在 Z.ai 認可的 coding 工具內使用；
    以此端點跑本腳本是否符合方案條款，請自行向 Z.ai 官方確認。"""
    try:
        import anthropic  # type: ignore
    except ImportError:
        sys.exit("[錯誤] 未安裝 anthropic SDK。請執行：pip install -r skills/glm/requirements.txt")

    api_key = os.environ.get("GLM_API_KEY")
    if not api_key:
        sys.exit("[錯誤] 未設定環境變數 GLM_API_KEY。")
    base_url = os.environ.get("GLM_ANTHROPIC_BASE_URL", DEFAULT_ANTHROPIC_BASE_URL)
    client = anthropic.Anthropic(api_key=api_key, base_url=base_url)
    print(f"[資訊] Anthropic 相容端點（待驗證）：{base_url}，模型：{model}", file=sys.stderr)

    def text_of(msg) -> str:
        return "".join(getattr(b, "text", "") for b in msg.content)

    messages = [{"role": "user", "content": user_prompt}]
    r1 = client.messages.create(model=model, system=system_prompt, max_tokens=max_tokens,
    ...
    r2 = client.messages.create(model=model, system=system_prompt, max_tokens=max_tokens,
                                temperature=0.0, messages=messages)
    return report, text_of(r2)
...
def main() -> int:
    ap = argparse.ArgumentParser(description="GLM 先制型曝險分析 API 工作流程")
    ...
    ap.add_argument("--model", default=os.environ.get("GLM_MODEL", DEFAULT_MODEL),
                    help="模型 ID（預設取 GLM_MODEL 或 glm-5.3；請以官方列表確認）")
    ...
    if args.dry_run:
        print("=" * 78)
        print(f"[DRY-RUN] 平台：GLM  模型：{args.model}")
        print(f"[DRY-RUN] OpenAI 相容端點：{os.environ.get('GLM_BASE_URL', DEFAULT_BASE_URL)}"
              f"（替代：{ALT_BASE_URL_CN}；請以官方文件確認）")
        print(f"[DRY-RUN] Anthropic 相容端點：{os.environ.get('GLM_ANTHROPIC_BASE_URL', DEFAULT_ANTHROPIC_BASE_URL)}（待驗證）")
    ...
    if args.anthropic_compatible:
        report, json_text = call_anthropic_compatible(system_prompt, user_prompt, json_prompt,
                                                      args.model, args.max_tokens)
    else:
        report, json_text = call_openai_compatible(system_prompt, user_prompt, json_prompt,
                                                   args.model, args.max_tokens)
