...
>
> 指令內容與模型後端無關；`metadata.backend: deepseek` 只是標記預期後端。任務定義、證據要求與人工核准邊界的單一來源是 `skills/shared/task-spec.md`；本檔不得與其牴觸。
...
## 核心指令（與 `skills/shared/core-prompt.md` 一致）
...
impact = clamp(business_criticality·1.4 + 資料等級權重{Restricted 3, Confidential 2, Internal 1} + 2·[crown jewel] + 2·[可達 crown jewel 之入口], 0, 10)
...
## 輸入檔對照（欄位細節見 `skills/shared/input-schema.json`）
...
以 `examples/synthetic-org/` 執行並對照 `skills/shared/task-spec.md` 第 7 節的 7 項檢查（筆數、`SYN-2026-0101` 為 P1、`internet → vpn-gw-01 → ad-dc-01 → erp-db-01` 路徑、驗證計畫皆標未授權、摘要 ≤ 300 字、移除情資後信心下修、移除 scope 後拒絕）。
