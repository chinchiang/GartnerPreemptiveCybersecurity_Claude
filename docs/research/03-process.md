...
| S4 | 攻擊路徑假設 Hypothesis | 從入口到 crown jewel 的路徑與可行性 | I10（＋ I6 身分邊、I8 阻斷控制）＋ S3 結果 | O2 攻擊路徑假設 | 架構師審查拓樸 |
| S5 | 改善建議 Remediation | 對 P1/P2 對映行動、負責人、工作量、驗證方式、核准層級；補償控制缺口 | S3、S4 結果 ＋ I3（修補可用性）、I7、I8 | O3 改善建議 | 變更委員會／OT 負責人 |
| S6 | 驗證計畫 Validation plan | 對前 N 條路徑提出驗證方法與授權需求（**只提案**） | S4 結果 ＋ I1、I10 | O4 安全驗證計畫 | CISO 書面授權 |
| S7 | 管理摘要 Mobilization | 面向受眾的摘要與決策請求 | S1–S6 結果 ＋ I1（受眾、遮罩） | O5 管理摘要 | 管理層決策 |
| S8 | 追蹤指標 Tracking | 指標與下次檢視 | S1–S7 結果 ＋ I2、I4（清冊完整度、未知資產） | O6 追蹤指標 | 每週檢視 |
...
- **輸出**：資料品質報告（JSON `data_quality`）；其中的清冊完整度估計與未納入清冊的對外資產數直接成為 O6 的兩項指標。
...
- **處理**：示範規則見 skills/shared/task-spec.md 附錄 A：可能性 = 對外曝露 + EASM 議題 + 公開利用 + EPSS/KEV + 身分弱點 + 設定偏差 + 控制缺口 − 既有控制 + 情資命中；影響 = 業務重要性 + 資料等級 + crown jewel + 可達 crown jewel 之入口（blast radius）；每項列 `factors[]`；依風險胃納分 P1–P4；信心水準依缺漏類別數與清冊完整度下修。
- **輸出**：O1。
...
- **輸入**：I10（crown jewels、edges）＋ S3 的節點可能性；I6 提供身分邊的依據、I8 提供可能阻斷控制。
...
- **輸入**：S3、S4 的結果；I3（`patch_available`）、I7（設定議題）、I8（補償控制缺口）。
...
- **輸入**：S4 的結果；I1（兩個授權旗標、排除項）、I10（判斷入口是否對外）。
- **處理**：`active_testing_authorized` 或 `external_scanning_authorized` 任一為 false，每項標「尚未授權主動測試」；涉及對外驗證的項目需兩旗標皆 true；OT 資產一律排除主動測試。
- **輸出**：O4。
...
- **目的**：讓管理層做決策，而不是看技術清單。
- **輸入**：S1–S6 的結果；I1 的 `reporting.audience` 與 `mask_identifiers`。
- **處理**：≤ 300 字；含現況、最急迫、決策請求、限制；預設遮罩識別資訊。
- **輸出**：O5。
- **限制**：摘要是模型撰寫，需人工校對。
- **人工決策點**：管理層決策。
- **依據**：CTEM mobilization「communication standards」[G8]；Gartner 新聞稿轉述（非逐字，待原頁核對）建議 CISO 以「protection levels」與「buying down exposure levels」的語言向管理層溝通 [G-SRM25]。

### S8 追蹤指標（Tracking）

- **目的**：讓流程可持續、可衡量。
- **輸入**：S1–S7 的結果；I2、I4（清冊完整度與未知對外資產）。
