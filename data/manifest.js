// 由 scripts/build-data.mjs 自動產生，請勿手動編輯。
window.BUILD_MANIFEST = {
 "builtAt": "2026-09-09T23:24:16.271Z",
 "research": [
  {
   "id": "00-method",
   "title": "0. 研究方法、證據分級與限制",
   "path": "docs/research/00-method.md"
  },
  {
   "id": "01-definition",
   "title": "1. 定義、目標、適用範圍與傳統偵測回應的關係",
   "path": "docs/research/01-definition.md"
  },
  {
   "id": "02-relations",
   "title": "2. 與 CTEM、威脅情資、攻擊面管理、曝險管理、攻擊路徑分析、安全驗證及欺敵技術的關聯",
   "path": "docs/research/02-relations.md"
  },
  {
   "id": "03-process",
   "title": "3. 可執行流程：八個步驟的目的、輸入、處理、輸出、限制與人工決策點",
   "path": "docs/research/03-process.md"
  },
  {
   "id": "04-inputs",
   "title": "4. 必要與選用的 Inputs",
   "path": "docs/research/04-inputs.md"
  },
  {
   "id": "05-outputs",
   "title": "5. 可能的 Outputs",
   "path": "docs/research/05-outputs.md"
  },
  {
   "id": "06-case",
   "title": "6. 完整案例：Northwind Precision（合成資料）",
   "path": "docs/research/06-case.md"
  },
  {
   "id": "07-platforms",
   "title": "7. 五個平台的查證摘要與等效方案",
   "path": "docs/research/07-platforms.md"
  }
 ],
 "guides": [
  {
   "id": "chatgpt",
   "title": "ChatGPT 安裝與使用教學",
   "path": "docs/guides/chatgpt.md"
  },
  {
   "id": "claude",
   "title": "Claude 安裝與使用教學",
   "path": "docs/guides/claude.md"
  },
  {
   "id": "grok",
   "title": "Grok 安裝與使用教學",
   "path": "docs/guides/grok.md"
  },
  {
   "id": "glm",
   "title": "GLM 安裝與使用教學",
   "path": "docs/guides/glm.md"
  },
  {
   "id": "deepseek",
   "title": "DeepSeek 安裝與使用教學",
   "path": "docs/guides/deepseek.md"
  },
  {
   "id": "deployment",
   "title": "部署與維護",
   "path": "docs/guides/deployment.md"
  }
 ],
 "skills": [
  {
   "path": "skills/chatgpt/README.md",
   "platform": "chatgpt",
   "bytes": 4946
  },
  {
   "path": "skills/chatgpt/api-workflow.py",
   "platform": "chatgpt",
   "bytes": 12992
  },
  {
   "path": "skills/chatgpt/codex-skill/SKILL.md",
   "platform": "chatgpt",
   "bytes": 9870
  },
  {
   "path": "skills/chatgpt/custom-gpt-instructions.md",
   "platform": "chatgpt",
   "bytes": 8322
  },
  {
   "path": "skills/chatgpt/project-instructions.md",
   "platform": "chatgpt",
   "bytes": 4337
  },
  {
   "path": "skills/chatgpt/requirements.txt",
   "platform": "chatgpt",
   "bytes": 12
  },
  {
   "path": "skills/claude/README.md",
   "platform": "claude",
   "bytes": 3257
  },
  {
   "path": "skills/claude/preemptive-exposure-analysis/SKILL.md",
   "platform": "claude",
   "bytes": 6045
  },
  {
   "path": "skills/claude/preemptive-exposure-analysis/references/acceptance.md",
   "platform": "claude",
   "bytes": 1445
  },
  {
   "path": "skills/claude/preemptive-exposure-analysis/references/input-schema.json",
   "platform": "claude",
   "bytes": 6539
  },
  {
   "path": "skills/claude/preemptive-exposure-analysis/references/output-schema.json",
   "platform": "claude",
   "bytes": 6310
  },
  {
   "path": "skills/claude/preemptive-exposure-analysis/references/scoring-rules.md",
   "platform": "claude",
   "bytes": 3309
  },
  {
   "path": "skills/claude/preemptive-exposure-analysis/scripts/validate_inputs.py",
   "platform": "claude",
   "bytes": 5499
  },
  {
   "path": "skills/deepseek/README.md",
   "platform": "deepseek",
   "bytes": 3421
  },
  {
   "path": "skills/deepseek/api-workflow.py",
   "platform": "deepseek",
   "bytes": 16097
  },
  {
   "path": "skills/deepseek/host-skill/SKILL.md",
   "platform": "deepseek",
   "bytes": 9831
  },
  {
   "path": "skills/deepseek/requirements.txt",
   "platform": "deepseek",
   "bytes": 12
  },
  {
   "path": "skills/deepseek/system-prompt.md",
   "platform": "deepseek",
   "bytes": 4969
  },
  {
   "path": "skills/glm/README.md",
   "platform": "glm",
   "bytes": 3415
  },
  {
   "path": "skills/glm/api-workflow.py",
   "platform": "glm",
   "bytes": 18155
  },
  {
   "path": "skills/glm/claude-code-settings.example.json",
   "platform": "glm",
   "bytes": 1145
  },
  {
   "path": "skills/glm/claude-code-skill/SKILL.md",
   "platform": "glm",
   "bytes": 9226
  },
  {
   "path": "skills/glm/requirements.txt",
   "platform": "glm",
   "bytes": 28
  },
  {
   "path": "skills/glm/system-prompt.md",
   "platform": "glm",
   "bytes": 4906
  },
  {
   "path": "skills/grok/README.md",
   "platform": "grok",
   "bytes": 3124
  },
  {
   "path": "skills/grok/api-workflow.py",
   "platform": "grok",
   "bytes": 9132
  },
  {
   "path": "skills/grok/requirements.txt",
   "platform": "grok",
   "bytes": 27
  },
  {
   "path": "skills/grok/system-prompt.md",
   "platform": "grok",
   "bytes": 4994
  },
  {
   "path": "skills/grok/tool-definitions.json",
   "platform": "grok",
   "bytes": 3159
  },
  {
   "path": "skills/shared/core-prompt.md",
   "platform": "shared",
   "bytes": 4176
  },
  {
   "path": "skills/shared/input-schema.json",
   "platform": "shared",
   "bytes": 6539
  },
  {
   "path": "skills/shared/output-schema.json",
   "platform": "shared",
   "bytes": 6310
  },
  {
   "path": "skills/shared/task-spec.md",
   "platform": "shared",
   "bytes": 10123
  }
 ],
 "downloads": [
  "downloads/skills-chatgpt.zip",
  "downloads/skills-claude.zip",
  "downloads/skills-deepseek.zip",
  "downloads/skills-glm.zip",
  "downloads/skills-grok.zip",
  "downloads/skills-shared.zip",
  "downloads/skills-all-platforms.zip",
  "downloads/synthetic-example-data.zip"
 ]
};
